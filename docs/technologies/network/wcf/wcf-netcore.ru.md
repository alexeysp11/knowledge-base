...
В .NET Core веб-приложения и сетевые приложения в подавляющем большинстве разрабатываются на основе ASP.NET Core. На текущий момент Microsoft не предоставляет других официальных альтернатив для веб-разработки на .NET Core/.NET 6+. Поэтому с огромной вероятностью при миграции WCF на более современные версии .NET мы будем использовать ASP.NET Core и инструменты, которые он предоставляет (например, Middleware, Dependency Injection и т.д.).

### Middleware как основа для построения WCF приложений на .NET Core

Middleware - это довольно мощный инструмент в ASP.NET Core, который позволяет разделить ответственность между разными компонентами приложения, а также значительно упростить разработку и поддержку ПО.
