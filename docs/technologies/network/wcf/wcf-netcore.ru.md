...
На текущий момент Microsoft не предоставляет других официальных альтернатив для веб-разработки на .NET Core/.NET 6+, кроме ASP.NET Core. Поэтому при миграции WCF на более современные версии .NET рекомендуется использовать именно ASP.NET Core и инструменты, которые он предоставляет (например, Middleware, Dependency Injection и т.д.).
...
**SoapCore.** Если используется большое количество нестандартных настроек WCF, то миграция на SoapCore может быть сложнее, и потребуется вручную реализовывать аналогичный функционал, т.к. SoapCore в основном ориентирован именно на протокол SOAP, а не на все возможности WCF (бинарные форматы, WS-, и т.д.).

### Архитектура

**CoreWCF.** Как уже было отмечено, отсутствие поддержки Dependency Injection в CoreWCF "из коробки" может требовать использование статических классов для хранения зависимостей, что "выключает" одну из основных функций ASP.NET Core. В результате может получиться, что один и тот же класс (зависимость) одновременно инициализируется/сохраняется как с помощью DI-контейнера, так и с помощью статического класса, что может вызвать проблемы в архитектуре.
...
Основываясь на результатах исследования, если используются базовые функции WCF, то SoapCore представляет собой более подходящий вариант для миграции WCF-сервиса на .NET 6+ в контексте архитектуры. Однако если требуются более специфичные функции WCF (бинарные форматы, WS-, и т.д.), то CoreWCF будет предоставлять чуть больший диапазон возможностей.
