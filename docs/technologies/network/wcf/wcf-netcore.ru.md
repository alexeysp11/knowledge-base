...
На текущий момент Microsoft не предоставляет других официальных альтернатив для веб-разработки и сетевых приложений на .NET Core/.NET 6+, кроме ASP.NET Core. Поэтому при миграции WCF на более современные версии .NET рекомендуется использовать именно ASP.NET Core и инструменты, которые он предоставляет (например, Middleware, Dependency Injection и т.д.).
