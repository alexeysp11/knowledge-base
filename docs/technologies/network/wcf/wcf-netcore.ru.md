...
- Сильная типизация:
    - Если нужна строгая типизация и автоматическая генерация кода, gRPC с Protobuf - идеальный вариант. Protobuf позволяет описать структуру данных, и на основе этих данных можно автоматически сгенерировать клиентский и серверный код.
- Использование HTTP/2:
    - Если используется HTTP/2, то стоит выбрать gRPC, так как он основан на HTTP/2.
...
- Простота:
    - Если нужна простота разработки и интеграции, то Web API - это более простой вариант. WCF (и его клоны) достаточно сложные в своей реализации.
...
- Микросервисы:
    - Если вы разрабатываете микросервисы, то стоит выбрать Web API.
